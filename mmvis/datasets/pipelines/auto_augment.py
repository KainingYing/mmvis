...
def bbox2fields():
    ...
    bbox2ids = {
        'gt_bboxes': 'gt_instance_ids',
    }
    return bbox2label, bbox2mask, bbox2seg, bbox2ids


@PIPELINES.register_module(force=True)
class COCO2Seq(Rotate):
    """"""
    def __init__(self, angle, num_frames=5, scale=1.0, img_fill_val=128):
        # assert isinstance(level, (int, float)), \
        #     'The level must be type int or float.'
        # assert 0 <= level <= _MAX_LEVEL, \
        #     'The level should be in range [0,_MAX_LEVEL].'
        ...
        self.scale = scale
        self.img_fill_val = img_fill_val
        self.angle_list = np.linspace(-angle,
                                      angle,
                                      num=num_frames,
                                      endpoint=True)
    ...
    def _filter_invalid(self, results, min_bbox_size=0):
        """Filter bboxes and corresponding masks too small after rotate
        augmentation."""
        bbox2label, bbox2mask, _, bbox2id = bbox2fields()
        for key in results.get('bbox_fields', []):
            bbox_w = results[key][:, 2] - results[key][:, 0]
            bbox_h = results[key][:, 3] - results[key][:, 1]
            valid_inds = (bbox_w > min_bbox_size) & (bbox_h > min_bbox_size)
            ...
            if mask_key in results:
                results[mask_key] = results[mask_key][valid_inds]
            id_key = bbox2id.get(key)
            if id_key in results:
                results[id_key] = results[id_key][valid_inds]

    def _rotate_single_image(self, results, angle):
        """Call function to rotate images, bounding boxes, masks and semantic
        segmentation maps.
    ...
    def __call__(self, results):
        ...
        assert len(
            results
        ) == 1, 'The length of results must be 1, because the results is a statistic image from coco'
        # if np.random.rand() > self.prob:
        #     return results
        results = [copy.deepcopy(results[0]) for _ in range(self.num_frames)]
        results = [
            self._rotate_single_image(results[i], angle)
