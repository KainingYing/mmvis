_base_ = [
    ...
    '../_base_/datasets/online_youtube_vis_2021.py',
    '../_base_/default_runtime.py'
]
model = dict(
    type='MaskTrackRCNN',
    detector=dict(
        roi_head=dict(bbox_head=dict(num_classes=40),
                      mask_head=dict(num_classes=40)),
        train_cfg=dict(rpn=dict(sampler=dict(num=64)),
                       rpn_proposal=dict(nms_pre=200, max_per_img=200),
                       rcnn=dict(sampler=dict(num=128))),
        test_cfg=dict(rpn=dict(nms_pre=200, max_per_img=200),
