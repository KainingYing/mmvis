...
from mmvis.datasets import build_dataloader, build_dataset
...
def measure_inference_speed(cfg, checkpoint, max_iter, log_interval,
    ...
    before_data_process = time.perf_counter()
    # benchmark with 2000 image and take the average
    for i, data in enumerate(data_loader):
        num_imgs = data['video'][0].shape[1]
        num_all_images += num_imgs
        ...
        process_elapsed = time.perf_counter() - before_data_process
        before_data_process = time.perf_counter()

        if i >= num_warmup:
            pure_inf_time += elapsed
            with_process_time += process_elapsed
            if (i + 1) % log_interval == 0:
                # print(num_all_images, num_warmup_images, pure_inf_time)
                forward_fps = (num_all_images -
                               num_warmup_images) / pure_inf_time
                with_process_fps = (num_all_images -
                                    num_warmup_images) / with_process_time
                print(
                    f'Done video [{i + 1:<3} / {max_iter}], '
                    f'forward fps: {forward_fps:.1f} img / s, '
                    f'forward&pre-processing fps: {with_process_fps:.1f} img / s, ',
                    flush=True)
        else:
            num_warmup_images += num_imgs
